...
def build_kg(root: Window, norm_pairs: List[Tuple[str, float]], experiment_folder: str, names_with_assigned_ages):
    ...
    with open(out_path, "w") as f, open(vals_path, "w") as vf:
        ...
        # --------- Write less than relationships on ages ---------- 04/01/26
        write_person_lt_rels = True

        if write_person_lt_rels:
            sorted_people = sorted(names_with_assigned_ages.items(), key=lambda x: x[1])
        ...
        # --------- Write same age relationships on people ----------
...
#create_kg(n_vertices=100, window_depth=4, experiment_folder="april_7_experiments/100people", decimal_precision=0, high=100, low=1, n_people=100)
#create_kg(n_vertices=100, window_depth=4, experiment_folder="april_7_experiments/500people", decimal_precision=0, high=100, low=1, n_people=500)
